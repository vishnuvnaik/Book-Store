...
describe("registerwrong", function () {
  ...
  it("should return status 422,  if allfields are null", function (done) {
    chai
      .request(server)
      .post("/register")
      .send(data.registerwrong4)
      .end((err, data) => {
        data.should.have.status(422);
        done();
      });
  });
  it("should return status 422,  if req body is null", function (done) {
    chai
      .request(server)
      .post("/register")
      .send(data.registerwrong5)
      .end((err, data) => {
        data.should.have.status(422);
        done();
      });
...
describe("login", function () {
  ...
  it("should return status 422,  if passwird doesnot have sufficient length", function (done) {
    chai
      .request(server)
      .post("/login")
      .send(data.loginWrong4)
      .end((err, data) => {
        data.should.have.status(422);
        done();
      });
  });
  it("should return status 500,  if password is wrong", function (done) {
    chai
      .request(server)
      .post("/login")
      .send(data.loginWrong5)
      .end((err, data) => {
        data.should.have.status(500);
        done();
      });
  });
  it("should return status 422,  if data is null", function (done) {
    chai
      .request(server)
      .post("/login")
      .send(data.loginWrong6)
      .end((err, data) => {
        data.should.have.status(422);
        done();
      });
...
describe("forgotPasswordwrong", function () {
  ...
  it("should return status 422, if mail doesnot exist", function (done) {
    chai
      .request(server)
      .post("/forgotPassword")
      .send(data.forgotpasswordwrong1)
      .end((err, data) => {
        data.should.have.status(200);
        done();
      });
